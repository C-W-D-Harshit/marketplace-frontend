...
const Beauty = ({ Dproducts, Nproducts }) => {
  return (
...
export async function getServerSideProps() {
  ...
  let data1 = [];
  try {
    res = await fetch(`${baseUrl}/api/v1/products/dod?category=beauty`);
    data = await res.json();
    res1 = await fetch(`${baseUrl}/api/v1/products/nA?category=beauty`);
